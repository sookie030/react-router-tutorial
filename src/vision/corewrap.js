...
exports.getAverageBlur_0409 = (imageStr) => {
  ...
  let result = ArrayType(uint8, size);
...
exports.getAverageBlur_0409_2 = (
  ...
  let image = new datatypes.ImageInfo({
    data: Buffer.from(data),
    size: new datatypes.SizeInfo({
      width: width,
      height: height,
    }),
    color: color,
  ...
  let imagePtr = ref.alloc(datatypes.ImageInfo, image);
  let test = Buffer.from(data);

  visionlib.get_average_blur(imagePtr, test);

  let test2 = Uint8Array.from(test);

  console.log(test2);

  let index1 = data.findIndex((elem) => {
    elem < 255;
  });
  let index2 = test2.findIndex((elem) => {
    elem < 255;
  });
 ...
 * @param {uint8*} edge_magnitude_vector (output) Edge magnitude
 */
 ...
 * @param {ImageInfo*} image Image info
 * @param {prewittOptionsPtr} options Options for the edge processing
 * @param {uint16*} edge_magnitude_vector (output) Edge magnitude
 * @param {uint16*} edge_gradient_vector (output) Edge gradient
 */
exports.getPrewittEdgeWithGradient = (imagePtr, optionsPtr, edgeMagnitudeVectorPtr) => {
  visionlib.get_prewitt_edge_with_gradient(imagePtr, optionsPtr, edgeMagnitudeVectorPtr);
}; 
...
/**
 * Run the robert edge processing
 * @param {ImageInfo*} image Image info
 ...
 * @param {uint16*} edge_gradient_vector (output) Edge gradient
 */
exports.getRobertsEdgeWithGradient = (imagePtr, optionsPtr, edgeMagnitudeVectorPtr) => {
  visionlib.get_roberts_edge_with_gradient(imagePtr, optionsPtr, edgeMagnitudeVectorPtr);
}; 
